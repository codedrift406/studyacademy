...
import { Play, Sparkles, Brain, Map as MapIcon, LayoutGrid, Hash, Trophy, Star, Zap } from 'lucide-react';
...
export const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([coursesRes, recommendationsRes, gamificationRes, leaderboardRes]) => {
    ...
    <div className={styles.container}>
          ...
          <CardBody className={styles.aiBody}>
            <div className={styles.aiContent}>
              <div className={styles.aiStatus}>
                ...
                <span className={styles.statusLabel}>{t('student.dashboard.aiTutorStatus')}</span>
              </div>
              <h2 className={styles.heroTitle}>
                {t('student.dashboard.heroTitle', 'Welcome back, {name}!', { name: user.name || (language === 'kk' ? 'Оқушы' : 'Ученик') })}
              </h2>
              <p className={styles.heroText}>
                {courses.length > 0
                  ? t('student.dashboard.heroSubtitle.courses', 'Your personalized learning paths are ready. Select a course to keep making progress.')
                  : t('student.dashboard.heroSubtitle.empty', 'You have not enrolled in any courses yet. Check the Course Catalog to begin your learning.')}
              </p>
              <div className={styles.heroStats}>
                <div className={styles.statChip}>
                  <Hash size={14} />
                  <span>{t('common.xp', 'XP: {xp}', { xp: user.xp || 0 })}</span>
                </div>
                <div className={styles.statChip}>
                  <Sparkles size={14} />
                  <span>{t('common.streakDays', '{days} days', { days: user.streakDays || 0 })}</span>
                </div>
              </div>
              <div className={styles.heroActions}>
                <Button variant="primary" icon={<Sparkles size={18} />} onClick={() => navigate('/student/catalog')}>
              ...
              <div className={styles.profileCard}>
                ...
                <div className={styles.xpProgressRow}>
                  <div className={styles.xpLabel}>
                    <span>{t('common.level', 'Level {level}', { level: gamification?.level || 1 })}</span>
      ...
      <section className={styles.sectionBlock}>
        <div className={styles.sectionHeader} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <h3>{t('student.dashboard.portfolioTitle', 'Your Learning Portfolio')}</h3>
            {isLoading && <span className={styles.subtleNote}>{t('student.dashboard.loadingDashboard', 'Updating dashboard...')}</span>}
          </div>
        ...
        {isLoading ? (
          ...
          <StudentRpgMap courses={courses.map((c, i) => ({
            id: c.id,
            title: c.title,
            progressPercent: c.progressPercent ?? 0,
            status: (c.progressPercent === 100) ? 'completed' : ((c.progressPercent ?? 0) > 0 || i === 0) ? 'active' : 'locked'
          })) as any} />
        ) : (
              ...
              <Card key={course.id} interactive>
                ...
                <CardBody>
                  <div className={styles.courseMetaRow}>
                    <span>Enrolled</span>
                    <span className={styles.progressLabel}>{course.progressPercent ?? 0}% complete</span>
                  </div>
                  <CardTitle className={styles.courseCardTitle}>{course.title}</CardTitle>
                  <p className={styles.courseTeacher}>{course.teacher_name || 'Instructor'}</p>
                  <div className={styles.progressBar}>
                    <div className={styles.progressFill} style={{ width: `${course.progressPercent ?? 0}%` }} />
                  </div>
                  {course.latestScore !== null && course.latestScore !== undefined && (
                    <p className={course.latestScore < 50 ? styles.warningText : styles.successText}>
                      Latest result: {course.latestScore}%
                    </p>
                  )}
                </CardBody>
                <CardFooter className={styles.cardFooterGroup}>
                  <Button fullWidth icon={<Play size={18} />} onClick={() => navigate(`/course/${course.id}`)}>
                    Resume Learning
                  </Button>
                </CardFooter>
              </Card>
      ...
      <section className={styles.gridSection}>
        <Card className={styles.recommendationCard}>
          <CardBody>
            <div className={styles.sectionHeader}>
              <h3>{t('student.dashboard.recommendationsTitle', 'Your Next Actions')}</h3>
            </div>
            {isLoading ? (
              <div className={styles.emptyState}>Loading recommendations...</div>
            ) : !recommendations?.actionPlan?.length ? (
              <div className={styles.emptyState}>{t('student.dashboard.recommendationsEmpty', 'Complete a lesson or quiz to unlock a personalized action plan.')}</div>
            ) : (
              recommendations.actionPlan.map((item) => (
                <div key={item.courseId} className={styles.recommendationItem}>
                  ...
                  <p className={styles.recommendationMeta}>
                    Progress: {item.progressPercent}%{item.averageGrade !== null ? ` • Grade: ${item.averageGrade}%` : ''}
                  </p>
                </div>
              ))
            )}
        ...
        <Card className={styles.recommendationCard}>
          <CardBody>
            <div className={styles.sectionHeader}>
              <h3>{t('student.dashboard.recommendedCoursesTitle', 'Recommended Next Courses')}</h3>
            </div>
            {isLoading ? (
              <div className={styles.emptyState}>Loading suggestions...</div>
            ) : !recommendations?.recommendedCourses?.length ? (
              <div className={styles.emptyState}>{t('student.dashboard.recommendedCoursesEmpty', 'New course suggestions will appear here as your learning profile grows.')}</div>
            ) : (
              recommendations.recommendedCourses.map((course) => (
                <div key={course.id} className={styles.recommendationItem}>
                  ...
                  <p className={styles.recommendationMeta}>{course.teacherName} • {course.lessonCount} lessons</p>
                  <div className={styles.recommendationAction}>
                    <Button size="sm" variant="secondary" onClick={() => navigate('/student/catalog')}>
                      Open Catalog
                    </Button>
                  </div>
                </div>
      ...
      <Card className={styles.focusCard}>
        <CardBody>
          ...
          {!recommendations?.focusAreas?.length ? (
            <div className={styles.emptyState}>{t('student.dashboard.focusAreasEmpty', 'Your weak-topic analysis will appear here after quiz attempts.')}</div>
          ) : (
