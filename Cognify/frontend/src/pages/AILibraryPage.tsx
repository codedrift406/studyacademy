        ...
        >
          ...
          <div className={styles.searchWrapper}>
            ...
            <div className={styles.mainSearchInput}>
              ...
              >
                {loading ? (
                  <Sparkles className={styles.spinning} size={20} />
                ) : (
                  <ArrowRight size={20} />
                )}
              </button>
              ...
              {[
                {
                  id: "search",
                  icon: <Search size={14} />,
                  label: t("ai.library.mode.search"),
                },
                {
                  id: "web_search",
                  icon: <Globe size={14} />,
                  label: t("landing.nav.solutions"),
                },
                {
                  id: "translate",
                  icon: <Languages size={14} />,
                  label: t("ai.library.mode.translate"),
                },
                {
                  id: "summarize",
                  icon: <FileText size={14} />,
                  label: t("ai.library.mode.summarize"),
                },
              ].map((chip) => (
                <button
                  key={chip.id}
...
const ResultsArea = React.memo(
  ({ results, loading, copied, onCopy, t }: any) => {
    return (
        ...
        {!results && !loading ? (
          ...
          >
            {(results?.translated || results?.summary) && (
              <div
                className={`${styles.resultCard} ${styles.specialCard} glass-panel`}
              >
                <div className={styles.cardHeader}>
                  ...
                  >
                    ...
                    <h4 className={styles.cardTitle}>
                      {results?.translated
                        ? t("ai.library.results.translation")
                        : t("ai.library.results.summary")}
                    </h4>
                  </div>
                  <button
                    className={styles.copyBtn}
                    onClick={() =>
                      onCopy(results?.translated || results?.summary || "")
                    }
                  >
                    {copied ? (
                    ...
                    <span>
                      {copied ? t("ai.library.copied") : t("ai.library.copy")}
                    </span>
                  </button>
                </div>
...
const AILibraryPage: React.FC = () => {
  ...
  const [mode, setMode] = useState<
    "search" | "translate" | "summarize" | "web_search"
  >("search");
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError(
        t(
          "ai.library.error",
          "Failed to load library results. Please try again.",
        ),
      );
